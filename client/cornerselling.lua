...
local function RobberyPed()
    if Config.UseTarget then
        exports['tmg-target']:AddEntityZone('stealingPed', stealingPed, {
            name = 'stealingPed',
            debugPoly = false,
        }, {
            options = {
                {
                    ...
                    action = function()
                        ...
                        TriggerServerEvent('tmg-drugs:server:giveStealItems', stealData.item, stealData.amount)
                        TriggerEvent('tmg-inventory:client:ItemBox', TMGCore.Shared.Items[stealData.item], 'add')
                        stealingPed = nil
                        stealData = {}
                        exports['tmg-target']:RemoveZone('stealingPed')
                    end,
                ...
                if IsEntityDead(stealingPed) then
                    ...
                    if not Config.UseTarget and #(pos - pedpos) < 1.5 then
                        if not textDrawn then
                            textDrawn = true
                            exports['tmg-core']:DrawText(Lang:t('info.pick_up_button'))
                        end
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            if getRobbed <= Config.RobberyChance then
                TriggerServerEvent('tmg-drugs:server:robCornerDrugs', drugType, bagAmount)
                TMGCore.Functions.Notify(Lang:t('info.has_been_robbed', { bags = bagAmount, drugType = availableDrugs[drugType].label }))
                stealingPed = ped
                stealData = {
                ...
                if pedDist2 < 1.5 and cornerselling then
                    if Config.UseTarget and not zoneMade then
                        zoneMade = true
                        exports['tmg-target']:AddEntityZone('sellingPed', ped, {
                            name = 'sellingPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                                            TMGCore.Functions.Notify(Lang:t('error.in_vehicle'), 'error')
                                            hasTarget = false
                                            ...
                                            exports['tmg-target']:RemoveZone('sellingPed')
                                            TMGCore.Functions.Progressbar('cornerSelling', Lang:t('info.selling_to_ped'), '5000', false, false, {
                                                disableMovement = true,
                                                disableCarMovement = true,
                                                disableMouse = false,
                                                disableCombat = false,
                                            }, {}, {}, {}, function()
                                                TriggerServerEvent('tmg-drugs:server:sellCornerDrugs', drugType, bagAmount, randomPrice)
                                                hasTarget = false
                                ...
                                },
                                {
                                    ...
                                    label = 'Decline offer',
                                    action = function(entity)
                                        TMGCore.Functions.Notify(Lang:t('error.offer_declined'), 'error')
                                        hasTarget = false
                                        SetPedKeepTask(entity, false)
                                        SetEntityAsNoLongerNeeded(entity)
                                        ClearPedTasksImmediately(entity)
                    ...
                    elseif not Config.UseTarget then
                        if not textDrawn then
                            textDrawn = true
                            exports['tmg-core']:DrawText(Lang:t('info.drug_offer', { bags = bagAmount, drugLabel = currentOfferDrug.label, randomPrice = randomPrice }))
                        end
...
local function ToggleSelling()
    if not cornerselling then
        ...
        TMGCore.Functions.Notify(Lang:t('info.started_selling_drugs'))
        ...
        CreateThread(function()
            while cornerselling do
                ...
                if not hasTarget then
                    ...
                    local closestPed, closestDistance = TMGCore.Functions.GetClosestPed(coords, PlayerPeds)
        ...
        cornerselling = false
        LocalPlayer.state:set('inv_busy', false, true)
        TMGCore.Functions.Notify(Lang:t('info.stopped_selling_drugs'))
    end
end
        ...
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                if result then
                    ...
                    TMGCore.Functions.Notify(Lang:t('error.has_no_drugs'), 'error')
                    LocalPlayer.state:set('inv_busy', false, true)
                end
            end
        else
            TMGCore.Functions.Notify(Lang:t('error.not_enough_police', { polices = Config.MinimumDrugSalePolice }), 'error')
        end
    end)
end)
...
RegisterNetEvent('tmg-drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if availableDrugs == nil or #availableDrugs <= 0 then
        TMGCore.Functions.Notify(Lang:t('error.no_drugs_left'), 'error')
        cornerselling = false
        LocalPlayer.state:set('inv_busy', false, true)
    end
end)
