...
AddStateBagChangeHandler('isLoggedIn', nil, function(_, _, value)
    if value then
        TMGCore.Functions.TriggerCallback('tmg-drugs:server:RequestConfig', function(DealerConfig)
...
local function KnockDoorAnim(home)
    ...
    if home then
        ...
        exports['tmg-core']:DrawText(Lang:t('info.other_dealers_button'), 'left')
        AwaitingInput()
    else
        TriggerServerEvent('InteractSound_SV:PlayOnSource', 'knock_door', 0.2)
        Wait(100)
...
local function RandomDeliveryItemOnRep()
    local myRep = TMGCore.Functions.GetPlayerData().metadata['rep']['dealer'] or 0
    ...
    if not waitingDelivery then
        ...
        if Config.NearbyDeliveries == true then
            ...
            if #nearbyLocations > 0 then
                ...
                TMGCore.Functions.Notify(Lang:t('error.delivery_fail'), 'error')
                if Config.Debug == true then
        ...
        TMGCore.Functions.Notify(Lang:t('info.sending_delivery_email'), 'success')
        TriggerServerEvent('tmg-drugs:server:giveDeliveryItems', waitingDelivery)
        SetTimeout(2000, function()
            TriggerServerEvent('tmg-phone:server:sendNewMail', {
                sender = Config.Dealers[currentDealer]['name'],
                subject = 'Delivery Location',
                message = Lang:t('info.delivery_info_email', { itemAmount = amount, itemLabel = TMGCore.Shared.Items[waitingDelivery['itemData']['item']]['label'] }),
                button = {
                    enabled = true,
                    buttonEvent = 'tmg-drugs:client:setLocation',
                    buttonData = waitingDelivery
                }
            })
        end)
    else
        TMGCore.Functions.Notify(Lang:t('error.pending_delivery'), 'error')
    end
end
...
local function DeliverStuff()
    if deliveryTimeout > 0 then
        ...
        PoliceCall()
        TMGCore.Functions.Progressbar('work_dropbox', Lang:t('info.delivering_products'), 3500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('tmg-drugs:server:successDelivery', activeDelivery, true)
            activeDelivery = nil
            if Config.UseTarget then
                exports['tmg-target']:RemoveZone('drugDeliveryZone')
            else
        ...
        TriggerServerEvent('tmg-drugs:server:successDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
        ...
        while waitingKeyPress do
            ...
            elseif dealerIsHome then
                if IsControlJustPressed(0, 38) then
                    GetClosestDealer()
                    TriggerServerEvent('tmg-drugs:server:dealerShop', currentDealer)
                    exports['tmg-core']:KeyPressed()
...
function InitZones()
    ...
    if Config.UseTarget then
        for k, v in pairs(Config.Dealers) do
            exports['tmg-target']:AddBoxZone('dealer_' .. k, vector3(v.coords.x, v.coords.y, v.coords.z), 1.5, 1.5, {
                name = 'dealer_' .. k,
                heading = v.heading,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
        ...
        dealerCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if not dealerIsHome then
                    exports['tmg-core']:DrawText(Lang:t('info.knock_button'), 'left')
                    AwaitingInput()
                elseif dealerIsHome then
                    exports['tmg-core']:DrawText(Lang:t('info.other_dealers_button'), 'left')
                    AwaitingInput()
...
RegisterNetEvent('tmg-drugs:client:RefreshDealers', function(DealerData)
    if not Config.UseTarget and dealerCombo then dealerCombo:destroy() end
...
RegisterNetEvent('tmg-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('tmg-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('tmg-drugs:client:setDealerItems', function(itemData, amount, dealer)
    Config.Dealers[dealer]['products'][itemData.slot].amount = Config.Dealers[dealer]['products'][itemData.slot].amount - amount
end)

RegisterNetEvent('tmg-drugs:client:setLocation', function(locationData)
    ...
    SetMapBlip(activeDelivery['coords'].x, activeDelivery['coords'].y)
    if Config.UseTarget then
        exports['tmg-target']:AddBoxZone('drugDeliveryZone', vector3(activeDelivery['coords'].x, activeDelivery['coords'].y, activeDelivery['coords'].z), 1.5, 1.5, {
        ...
        drugDeliveryZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                exports['tmg-core']:DrawText(Lang:t('info.deliver_items_button', { itemAmount = activeDelivery['amount'], itemLabel = TMGCore.Shared.Items[activeDelivery['itemData']['item']]['label'] }), 'left')
                CreateThread(function()
...
RegisterNetEvent('tmg-drugs:client:sendDeliveryMail', function(type, deliveryData)
